...
class Block(nn.Module):
    def __init__(self, n_emd, num_heads, dropout):
        super().__init__()
        self.sa = MultiHeadAttention(num_heads, n_emd)
        self.ffwd = FeedForward(n_emd)
        self.norm1 = nn.LayerNorm(n_emd)
        self.norm2 = nn.LayerNorm(n_emd)
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads, n_emd):
        super().__init__()
        self.head_emd = n_emd // num_heads
        self.heads = nn.ModuleList([Head(n_embd, self.head_emd) for _ in range(num_heads)])
...
class Head(nn.Module):
    def __init__(self, n_emd, head_emd):
        super().__init__()
        self.key = nn.Linear(n_emd, head_emd, bias=False)
        self.query = nn.Linear(n_emd, head_emd, bias=False)
        self.value = nn.Linear(n_emd, head_emd, bias=False)
...
class BingramLanguageModel(nn.Module):
    ...
    def __init__(self, vocab_size, block_size, n_emd, dropout):
        super().__init__()
...
if __name__ == '__main__':
    ...
    with open('F:\githubProject\AiPremiumClass\梁锐江\week16\急难愁盼文件.txt', 'r', encoding='utf-8') as f:
        text = f.read()
    ...
    # 引入大模型
    model = BingramLanguageModel(vocab_size, block_size, n_embd, dropout)
    model.to(device)
